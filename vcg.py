...
class VCG:
    ...
    def compute(slot_clicks, reserve, bids):
        ...
        def total_payment(k):
            ...
            # unallocated => payment is 0
            if k >= n:
                payment = 0

            # if we are computing payment of the last spot
            elif k == n - 1:
                # if we have more valid bids than allocated spots, charge b_k+1 else charge 4
                payment = valid_bids[k + 1][1] if n < len(valid_bids) else reserve
                payment = payment * c[k]

            # if we are computing payment of any other spot
            else:
                # compute recursively
        ...
        # total_payments = [total_payment(k) for k in range(len(allocation))]
